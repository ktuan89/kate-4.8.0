...
void KateViewInternal::slotShowFontInfo() {
  renderer()->showFontInfo();
}

void KateViewInternal::slotToggle111() {
  renderer()->toggleAntialiasing();
}

void KateViewInternal::slotToggle222() {
  renderer()->toggleTextAntialiasing();
}

void KateViewInternal::slotToggle333() {
  renderer()->toggleSmoothPixmapTransform();
}

void KateViewInternal::slotToggle444() {
  renderer()->toggleHighQualityAntialiasing();
}

void KateViewInternal::slotIncFontWeight()
{
  renderer()->increaseFontWeight();
}

void KateViewInternal::slotDecFontWeight()
{
  renderer()->decreaseFontWeight();
}

void KateViewInternal::slotIncFontSizesWithStrategy() {
  renderer()->increaseFontSizesWithStrategy();
}
...
bool KateViewInternal::eventFilter( QObject *obj, QEvent *e )
{
  ...
  switch( e->type() )
  {
    ...
    case QEvent::ShortcutOverride:
    {
      ...
      if (m_alphabetamode && k->modifiers() == Qt::MetaModifier &&
          (k->key() == Qt::Key_I || k->key() == Qt::Key_K || k->key() == Qt::Key_J || k->key() == Qt::Key_L))
      {
        k->accept();
        return true;
      }
    ...
    {
      /* int db_area = KDebug::registerArea("ktuan-debug");
      kDebug(db_area) << "Key press event filter"; */
      QKeyEvent *k = static_cast<QKeyEvent *>(e);
      ...
      if (obj == this && (!k->modifiers() || k->modifiers() == Qt::ShiftModifier)) {
        // kDebug(db_area) << "call key press event";
        keyPressEvent( k );
        if (k->isAccepted()) {
          // kDebug(db_area) << " and get accepted";
...
{
  /*int db_area = KDebug::registerArea("ktuan-debug");
  kDebug(db_area) << "Key press event " << e->key();
  kDebug(db_area) << e->modifiers(); */
  ...
  if (m_alphabetamode) {
    const int key = e->key();
    const int modifiers = e->modifiers();
    bool ac_key = true;
    if (key == Qt::Key_J) {
      if (modifiers == Qt::MetaModifier) {
        home(m_kt_select);
      } else if (modifiers == Qt::ShiftModifier) {
        wordLeft(m_kt_select);
      } else {
        cursorLeft(m_kt_select);
      }
    }
      ...
      if (modifiers == Qt::MetaModifier) {
        ...
        wordRight(m_kt_select);
      } else {
        cursorRight(m_kt_select);
      }
    }
    else if (key == Qt::Key_K) {
      if (modifiers == Qt::MetaModifier) {
        pageDown(m_kt_select);
      } else {
        cursorDown(m_kt_select);
      }
    }
      ...
      if (modifiers == Qt::MetaModifier) {
        ...
        cursorUp(m_kt_select);
      }
    }
    else if (key == Qt::Key_S) {
      m_kt_select = !m_kt_select;
    }
      ...
      doReturn();
      m_view->emitToggleAlphaBetaMoveMode();
