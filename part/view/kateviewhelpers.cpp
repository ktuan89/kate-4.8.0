...
void KateCmdLineEdit::slotReturnPressed ( const QString& text )
{
  ...
  if (cmd.length () > 0)
  {
    ...
    // ktuan: need to change this list if my command changes view
    ...
    if (QRegExp("buffer|b|new|vnew|bp|bprev|bn|bnext|bf|bfirst|bl|blast").exactMatch(cmd.split(" ").at(0))
      || cmd.startsWith("exeCommand o ") || cmd.startsWith("exeCommand oi")
    ){
      emit hideRequested();
    }
  ...
  if (!(
    QRegExp("buffer|b|new|vnew|bp|bprev|bn|bnext|bf|bfirst|bl|blast").exactMatch(cmd.split(" ").at(0))
    || cmd.startsWith("exeCommand o ") || cmd.startsWith("exeCommand oi")
  )) {
    m_view->setFocus ();
  }
...
void KateIconBorder::paintBorder (int /*x*/, int y, int /*width*/, int height)
{
  ...
  int ktuan_part = -1;
  int ktuan_pos = -1;
  int ktuan_lines_once = m_viewInternal->cache()->lastViewLine(0);
  int db_area = KDebug::registerArea("ktuan-debug");
  // kDebug(db_area) << ktuan_lines_once;
  if (m_doc->lines() > 50) {
    int lines_once = (endz-startz+1);//ktuan_lines_once;
    // bar_length / lines_once = lines_once / lines
    int bar_length = lines_once * lines_once / m_doc->lines();
    bar_length = qMax(3, qMin(bar_length, lines_once * 2 / 3));
    if (bar_length % 2 == 0) bar_length++;
    int part = bar_length / 2;
    // center may fall between part -> lines_once - part - 1
    int len_centers = lines_once - 2 * part;
    // position of center
    // pos / ncen = doc_pos / lines
    int doc_pos = m_viewInternal->cache()->viewLine((startz + endz) / 2).line();
    int pos = len_centers * doc_pos / m_doc->lines();
    ktuan_part = part;
    ktuan_pos = pos + part;

    // kDebug(db_area) << m_doc->lines() << " " << startz << " " << endz;
    // kDebug(db_area) << lines_once << " " << bar_length << " " << part << " " << len_centers << " " << pos;
  }
  ...
  for (uint z=startz; z <= endz; z++)
  {
    ...
    if (ktuan_pos != -1 && z >= ktuan_pos - ktuan_part && z <= ktuan_pos + ktuan_part) {
      p.fillRect( w-7, y, 3, h, QColor(113, 96, 131));
    }
    // icon pane
    if( m_iconBorderOn )
    {
