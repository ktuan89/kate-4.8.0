...
{
  /*int db_area = KDebug::registerArea("ktuan-debug");
  kDebug(db_area) << "reset font here";
  QFont f ( config()->font () );
...
void KateRenderer::increaseFontWeight()
{
  QFont f ( config()->font () );
  int z = f.weight();
  int db_area = KDebug::registerArea("ktuan-debug");
  kDebug(db_area) << "increase font weight " << z;
  if (z + 5 < 100) {
    f.setWeight(z + 5);
    config()->setFont (f);
  }
...
void KateRenderer::decreaseFontWeight()
{
  QFont f ( config()->font () );
  int z = f.weight();
  int db_area = KDebug::registerArea("ktuan-debug");
  kDebug(db_area) << "decrease font weight " << z;
  if (z - 5 >= 0) {
    f.setWeight(z - 5);
    config()->setFont (f);
  }
...
void KateRenderer::showFontInfo() {
  QFontInfo info(config()->font());
  int db_area = KDebug::registerArea("ktuan-debug");
  kDebug(db_area) << "Family: " << info.family();
  kDebug(db_area) << "Fixed Pitch: " << info.fixedPitch();
  kDebug(db_area) << "pixelSize: " << info.pixelSize();
  kDebug(db_area) << "pointSize: " << info.pointSize();
  kDebug(db_area) << "weight: " << info.weight();
}

void KateRenderer::increaseFontSizesWithStrategy() {
  QFont f ( config()->font () );
  if ((int)f.styleStrategy() == QFont::PreferDefault) {
...
void KateRenderer::paintTextLineBackground(QPainter& paint, KateLineLayoutPtr layout, int currentViewLine, int xStart, int xEnd)
{
  ...
  /*{
    ...
    double ktuan_opac;
    int ktuan_alpha, __;
    fi >> ktuan_opac >> __ >> __ >> ktuan_alpha;
    fi.close();
    if (ktuan_alpha >= 0) {
      backgroundColor.setAlpha(ktuan_alpha);
      currentLineColor.setAlpha(ktuan_alpha);
    }
  }*/
...
//   kDebug( 13033 )<<"KateRenderer::paintTextLine";
  ...
  // KTUAN change
	...
	// QFont old_f = config()->font();
	// QFont f ( config()->font () );
	// f.setStyleStrategy(QFont::NoAntialias);
	// config()->setFont(f);
  // END

  /*paint.setRenderHint(QPainter::Antialiasing, m_Antialiasing);
  paint.setRenderHint(QPainter::TextAntialiasing, m_TextAntialiasing);
  paint.setRenderHint(QPainter::SmoothPixmapTransform, m_SmoothPixmapTransform);
  paint.setRenderHint(QPainter::HighQualityAntialiasing, m_HighQualityAntialiasing);*/
