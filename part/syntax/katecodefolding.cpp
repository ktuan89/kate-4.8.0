...
void KateCodeFoldingTree::deleteEndNode(KateCodeFoldingNode* deletedNode, bool delMem)
{
  ...
  // ktuan bug: sometime we delete parent before child, and deleting the child update the parent
  if (delMem) delete deletedNode;
}

// This method deletes a start node, "deletedNode"
void KateCodeFoldingTree::deleteStartNode(KateCodeFoldingNode* deletedNode, bool delMem)
{
  // step 0 - remove the node from map (unfold if necessary)
  deleteNodeFromMap(deletedNode);
...
void KateCodeFoldingTree::updateMapping(int line, const QVector<int> &newColumns, int virtualNodeIndex, int virtualColumn)
{
  ...
  for ( ; index_new < newColumns.size() ; index_new += 2 ) {
    ...
    // then we update the column (maybe the column has changed).
      ...
      // If the change of the column will change the nodes hierarchy, then we delete it
        ...
        // m_kt_tobeDeleted.append(oldLineMapping[index_old]);
        deleteNode(oldLineMapping[index_old], true);
        index_new -= 2;
      }
      index_old ++;
    }
  ...
  for (int i = index_old ; i < oldLineMapping.size() ; i ++) {
    // ktuan bug
    // m_kt_tobeDeleted.append(oldLineMapping[i]);
    deleteNode(oldLineMapping[i], true);
  }
