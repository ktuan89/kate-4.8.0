...
class KATEPART_TESTS_EXPORT KateCodeFoldingTree : public QObject
{
  ...
  // Used to save state (close / open ; reload)
    ...
    QVector<KateCodeFoldingNode*> m_kt_tobeDeleted;

  public:

    // ktuan
    void deleteKtNode() {
      for (int i = 0; i < m_kt_tobeDeleted.size(); ++i) {
        delete m_kt_tobeDeleted[i];
      }
  ...
  // Delete Node methods
    inline void deleteNode (KateCodeFoldingNode* deletedNode, bool delMem = true)
    {
      deletedNode->m_type > 0 ? deleteStartNode(deletedNode, delMem) : deleteEndNode(deletedNode, delMem);
    }
    void deleteEndNode (KateCodeFoldingNode* deletedNode, bool delMem);
    void deleteStartNode (KateCodeFoldingNode* deletedNode, bool delMem);
