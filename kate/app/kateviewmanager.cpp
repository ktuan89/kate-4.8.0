...
void KateViewManager::setupActions ()
{
  ...
  goNextSwitchable = m_mainWindow->actionCollection()->addAction( "go_next_switchable_split_view" );
  goNextSwitchable->setText( i18n("Next Switchable Split View") );
  goNextSwitchable->setShortcut( Qt::META + Qt::Key_S );
  connect(goNextSwitchable, SIGNAL(triggered()), this, SLOT(activateNextSwitchableView()));
  ...
  // KTUAN
  a = m_mainWindow->actionCollection()->addAction( "view_show_view_1" );
  a->setText("Show View Space 1");
  a->setShortcut(Qt::META + Qt::Key_1);
  connect(a, SIGNAL(triggered()), this, SLOT(showViewSpace1()));

  a = m_mainWindow->actionCollection()->addAction( "view_show_view_2" );
  a->setText("Show View Space 2");
  a->setShortcut(Qt::META + Qt::Key_2);
  connect(a, SIGNAL(triggered()), this, SLOT(showViewSpace2()));

  a = m_mainWindow->actionCollection()->addAction( "view_show_view_3" );
  a->setText("Show View Space 3");
  a->setShortcut(Qt::META + Qt::Key_3);
  connect(a, SIGNAL(triggered()), this, SLOT(showViewSpace3()));

  a = m_mainWindow->actionCollection()->addAction( "view_show_view_4" );
  a->setText("Show View Space 4");
  a->setShortcut(Qt::META + Qt::Key_4);
  connect(a, SIGNAL(triggered()), this, SLOT(showViewSpace4()));

  a = m_mainWindow->actionCollection()->addAction( "view_show_view_5" );
  a->setText("Show View Space 5");
  a->setShortcut(Qt::META + Qt::Key_5);
  connect(a, SIGNAL(triggered()), this, SLOT(showViewSpace5()));

  a = m_mainWindow->actionCollection()->addAction( "view_show_view_6" );
  a->setText("Show View Space 6");
  a->setShortcut(Qt::META + Qt::Key_6);
  connect(a, SIGNAL(triggered()), this, SLOT(showViewSpace6()));
...
void KateViewManager::updateViewSpaceActions ()
{
  ...
  goNextSwitchable->setEnabled (viewSpaceCount() > 1);
  goPrev->setEnabled (viewSpaceCount() > 1);

  for (int i = 0; i < m_viewSpaceList.count(); ++i) {
    m_viewSpaceList.at(i)->updateIndex(i);
  }
}
...
bool KateViewManager::deleteView (KTextEditor::View *view, bool delViewSpace)
{
  ...
  KateViewSpace *viewspace = (KateViewSpace *)view->parentWidget()->parentWidget()->parentWidget();
...
void KateViewManager::activateSpace (KTextEditor::View* v)
{
  ...
  KateViewSpace* vs = (KateViewSpace*)v->parentWidget()->parentWidget()->parentWidget();
  if (!vs->isActiveSpace())
  {
    setActiveSpace (vs);
    activateView(v);
...
void KateViewManager::activateNextSwitchableView() {
  int i = m_viewSpaceList.indexOf (activeViewSpace()) + 1;
  if (i >= m_viewSpaceList.count())
    i = 0;
  for (int j = 0; j + 1 < m_viewSpaceList.count(); ++j) {
    if (m_viewSpaceList.at(i)->canSwitch()) {
      setActiveSpace (m_viewSpaceList.at(i));
      activateView(m_viewSpaceList.at(i)->currentView());
      return;
    }
