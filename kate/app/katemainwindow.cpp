    ...
    : KateMDI::MainWindow (0)
{
  ...
  double ktuan_opac;
  int ktuan_Translucent, ktuan_alpha;
  fi >> ktuan_opac >> ktuan_Translucent >> ktuan_alpha;
  fi.close();
  setWindowOpacity(ktuan_opac);
  if (ktuan_Translucent == 1) setAttribute(Qt::WA_TranslucentBackground);
  if (ktuan_alpha >= 0) {
    ostringstream oss;
    oss << "background:rgba(200,200,200," << ktuan_alpha << ");";
    setStyleSheet(QString::fromUtf8(oss.str().c_str()));
  }
...
void KateMainWindow::setupActions()
{
  ...
  a = actionCollection()->addAction("view_alphabeta_move_mode");
...
void KateMainWindow::slotWindowActivated ()
{
  if (m_viewManager->activeView()) {
    updateCaption (m_viewManager->activeView()->document());
      ...
      connect(this, SIGNAL(signalUpdateAlphaBetaMoveMode(bool)),
              m_viewManager->activeView(), SLOT(updateAlphaBetaMoveMode(bool)),
              Qt::UniqueConnection);

      connect(this, SIGNAL(signalUpdateAlphaBetaMoveMode(bool)),
              m_viewManager->activeViewSpace(), SLOT(updateAlphaBetaMoveMode(bool)),
              Qt::UniqueConnection);
