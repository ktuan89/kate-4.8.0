...
bool PluginViewKateQuickDocumentSwitcherDialog::eventFilter(QObject *obj, QEvent *event) {
    if (event->type()==QEvent::KeyPress) {
        ...
        if (keyEvent->key() == Qt::Key_K && keyEvent->modifiers() == Qt::META) {
          QCoreApplication::sendEvent(m_listView, new QKeyEvent(QEvent::KeyPress, Qt::Key_Down, Qt::NoModifier));
          return true;
        }
        if (keyEvent->key() == Qt::Key_I && keyEvent->modifiers() == Qt::META) {
          QCoreApplication::sendEvent(m_listView, new QKeyEvent(QEvent::KeyPress, Qt::Key_Up, Qt::NoModifier));
          return true;
        }
        if (obj==m_inputLine) {
